...
<main class="main-content">
    <div class="container" style="max-width: 1200px; margin: 0 auto; padding: 0 20px;">
        ...
        <div class="filter-wrapper" style="display: flex; gap: 12px; margin-bottom: 30px; overflow-x: auto; padding-bottom: 10px;">
            ...
            // menjalankan instruksi query untuk mengambil daftar stand yang tersedia
            ...
            while ($s = mysqli_fetch_array($ambil_stand)) {
                ...
                // menentukan status aktif pada tombol filter berdasarkan parameter url
        ...
        <div class="menu-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 25px;">
            ...
            // inisialisasi variabel filter pencarian data
            $where_clause = "";

            // memeriksa apakah terdapat parameter filter stand pada url
            if (isset($_GET['stand'])) {
                $id_s = $_GET['stand'];
                $where_clause = "WHERE id_stand = '$id_s'";
            }

            // menjalankan instruksi query untuk mengambil data menu dengan filter dan pengurutan terbaru
            $ambil_menu = mysqli_query($conn, "SELECT * FROM menu $where_clause ORDER BY id_menu DESC");

            // mengambil data dari hasil query menjadi array
            while ($m = mysqli_fetch_array($ambil_menu)) {

                // memeriksa ketersediaan stok untuk menentukan tampilan kartu menu
                $is_habis = ($m['stok'] <= 0);
            ?>
